...
# import numpy as np
from numpy import zeros, hstack, linspace, arange, full, array, concatenate, std, sum, where, max, fft, sign, diff
from numpy import multiply, hamming, empty, mean, square
...
#     amp_datamatrix = np.hstack((extent_matrix,amp_datamatrix))
    extent_matrix = zeros((1020,8))
    amp_datamatrix = hstack((extent_matrix,amp_datamatrix))
...
#     fasttime = np.arange((area_start-1e-5), (area_end-1e-5)+bin_length, bin_length)
    slowtime = arange(sampletime)
    fasttime = arange((area_start-1e-5), (area_end-1e-5)+bin_length, bin_length)
    
    for i in slowtime:
#         slowtime_array = np.full(len(fasttime), i)
        slowtime_array = full(len(fasttime), i)
...
#         points = np.array([fasttime,slowtime_array,amp_array]).T.reshape(-1,1,3)
        points = array([fasttime,slowtime_array,amp_array]).T.reshape(-1,1,3)
...
#         avrmatrix[i] = np.sum(amp_matrix[ : ,i])/1020
        avrmatrix[i] = sum(amp_matrix[ : ,i])/1020
...
#     peakdiff_matrix = np.zeros((1020,13))
    peakarea_matrix = zeros((1020,13))
    peakdiff_matrix = zeros((1020,13))
...
#         index = np.where(amp_datamatrix[i]==np.max(slice_matrix))[0][0]
        index = where(amp_datamatrix[i]==max(slice_matrix))[0][0]
...
#     diff_matrix = np.sum(diff_matrix,axis=1)
    diff_matrix = sum(diff_matrix,axis=1)
...
#     fd_data = np.fft.rfft(filtered_matrix)
    fd_data = fft.rfft(filtered_matrix)

#     doublediff = np.diff(np.sign(np.diff(filtered_matrix)))
#     peak_locations = np.where(doublediff == -2)[0] + 1
    doublediff = diff(sign(diff(filtered_matrix)))
    peak_locations = where(doublediff == -2)[0] + 1
...
#     ax_x2 = np.arange(0, (fs/N)*((N/2)+1)*60, fs/N*60)
    ax_x = arange(1020)/17
    ax_x2 = arange(0, (fs/N)*((N/2)+1)*60, fs/N*60)
...
#     fd_data = np.fft.rfft(filtered_matrix)
    ...
    filtered_matrix = multiply(matrix_1d[:length*FPS],hamming(length*FPS))
    fd_data = fft.rfft(filtered_matrix)
...
#         fd_data += np.fft.rfft(filtered_matrix)
        filtered_matrix = multiply(matrix_1d[start:stop],hamming(length*FPS))
        fd_data += fft.rfft(filtered_matrix)
...
#     bin_range = np.arange(bin_start, bin_end)
    bin_range = arange(bin_start, bin_end)
    num = len(bin_range)

#     sum_matrix = np.zeros(1020)
    sum_matrix = zeros(1020)
    fig = plt.figure()
...
#     index = np.where(amp_datamatrix[0]==np.max(amp_datamatrix[0]))[0][0]
    ...
    peakline_matrix = zeros(1020)
    peakline_matrix[0] = max(amp_datamatrix[0])
    index = where(amp_datamatrix[0]==max(amp_datamatrix[0]))[0][0]
...
#         index = np.where(amp_datamatrix[i]==np.max(slice_matrix))[0][0]
        peakline_matrix[i] = max(slice_matrix)
        index = where(amp_datamatrix[i]==max(slice_matrix))[0][0]
...
#     fd_data = np.fft.rfft(sig_win)
    sig_win = multiply(rangebin_data[ 170:win_length*FPS+170], hamming(win_length*FPS))
    fd_data = fft.rfft(sig_win)
...
#     ax_x2 = np.arange(0, (fs/N)*((N/2)+1), fs/N)
    ax_x2 = arange(0, (fs/N)*((N/2)+1), fs/N)
...
#     print(np.where(fd_data == np.max(fd_data))[0][0])
    print(where(fd_data == max(fd_data))[0][0])
...
#         sub_matrix[i] = np.max(detect_matrix[:,i]) - np.min(detect_matrix[:,i])
        sub_matrix[i] = max(detect_matrix[:,i]) - min(detect_matrix[:,i])
#     sub_matrix = np.where(sub_matrix>threshold)[0]*bin_length + area_start
    sub_matrix = where(sub_matrix>threshold)[0]*bin_length + area_start
    print(sub_matrix)


#     ax_x = np.arange((area_start-1e-5), (area_end-1e-5)+bin_length, bin_length)
    ax_x = arange((area_start-1e-5), (area_end-1e-5)+bin_length, bin_length)
